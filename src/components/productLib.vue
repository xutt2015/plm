	...
	<el-row :gutter='20'>
	  <el-col :span='6'>
	    <el-input
  		...
  		<el-tree  id='productLibTree' :data='treeData'  :props='defaultProps' :load='loadNode' check-strictly show-checkbox  default-expand-all node-key='id' :expand-on-click-node='false' :render-content='renderContent'  @check-change='handleCheckChange' @node-click='handleNodeClick'></el-tree>
	  </el-col>
	  <el-col :span='18'>
      <el-tabs v-model='activeName' @tab-remove='removeTab'  @tab-click='tabChange'>
        <el-tab-pane label='产品' name='product'> 
          <el-table :data='tableData' border style='width: 100%' :default-sort='{prop:"validTime", order:"descending"}'>
            ...
            <el-table-column prop='partName' label='零部件名称' sortable width='200'>
            </el-table-column>
            ...
            <el-table-column prop='partCode' label='零部件编号' sortable width='180'>
            </el-table-column>
        ...
        <el-tab-pane label='零件信息' name='PartInfo'>零件信息</el-tab-pane>
    ...
    addTab (targetName, index, row) {
      ...
      if (isNewTab) {
        this.editableTabs.push({
          title: row.partName,
          name: row.partCode,
          content: 'tab' + row.partName + ':'
        })
      }
      this.activeName = row.partCode
    },
    // 关闭Tab页
